...
def _decode_via_channel(
    ...
    *,
    ...
    if channel == "exo":
        # ruff:ignore[lambda-assignment]
        predict_y = lambda obs: model.predict_y_exo(obs, streams=streams, inference=True)
    elif channel == "endo":
        # ruff:ignore[lambda-assignment]
        predict_y = lambda obs: model.predict_y_endo(obs, streams=streams, inference=True)
    else:
